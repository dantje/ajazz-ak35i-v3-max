...
static void cmd_solid(const std::vector<std::string>& args, bool verbose, bool quiet)
{
    ...
    for (int i = 0; i < static_cast<int>(args.size()); ++i) {
        ...
        if (eq(a, "-h", "--help")) {
            std::cout <<
                "Usage: ajazz solid [-r R] [-g G] [-b B] [--slot N] [--no-save]\n"
                "\n"
                ...
                "  --slot <1-10>        LCD slot index (default: 1)\n"
                "  --no-save            Volatile upload (may still persist on power cycle)\n"
                "\n"
                "Examples:\n"
                "  ajazz solid\n"
        ...
        else if (take_opt(args, i, nullptr, "--slot", val)) slot = parse_int("--slot", val, 1,  10);
        else if (eq(a, "--no-save")) no_save = true;
        else die("unknown option: " + args[i]);
    }
    ...
    struct Ctx { const uint8_t* data; size_t size; uint8_t slot; bool save; };
    Ctx ctx{ pixels.data(), pixels.size(), static_cast<uint8_t>(slot), !no_save };

    open_and_run(verbose, quiet,
        [](ajazz::Keyboard& kbd, const void* p) {
            auto& c = *static_cast<const Ctx*>(p);
            kbd.send_image(c.data, c.size, c.slot, c.save, /*add_header=*/true);
        }, &ctx);
}
...
static void cmd_image(const std::vector<std::string>& args, bool verbose, bool quiet)
{
    ...
    for (int i = 0; i < static_cast<int>(args.size()); ++i) {
        ...
        if (eq(a, "-h", "--help")) {
            std::cout <<
                "Usage: ajazz image <file> [--fill] [--slot N] [--no-save]\n"
                "\n"
    ...
    open_and_run(verbose, quiet,
        [](ajazz::Keyboard& kbd, const void* p) {
            auto& c = *static_cast<const Ctx*>(p);
            kbd.send_image(c.data, c.size, c.slot, c.save, /*add_header=*/false);
        }, &ctx);
}
...
static void cmd_animation(const std::vector<std::string>& args, bool verbose, bool quiet)
{
    ...
    for (int i = 0; i < static_cast<int>(args.size()); ++i) {
        ...
        if (eq(a, "-h", "--help")) {
            std::cout <<
                "Usage: ajazz animation <file> [--fill] [--slot N]\n"
                "\n"
    ...
    struct Ctx { const uint8_t* data; size_t size; uint8_t slot; };
    Ctx ctx{ pixels.data(), pixels.size(), static_cast<uint8_t>(slot) };

    open_and_run(verbose, quiet,
        [](ajazz::Keyboard& kbd, const void* p) {
            auto& c = *static_cast<const Ctx*>(p);
            kbd.send_image(c.data, c.size, c.slot, /*save=*/true, /*add_header=*/false);
        }, &ctx);
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (eq(a, "-h", "--help"))    { usage_global(); return 0; }
        else if (a[0] == '-')              die("unknown option: " + std::string(a));
